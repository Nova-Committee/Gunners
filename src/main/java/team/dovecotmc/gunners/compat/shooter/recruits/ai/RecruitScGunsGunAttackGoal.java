...
import team.dovecotmc.gunners.compat.ai.ScGunsAttackGoal;

public class RecruitScGunsGunAttackGoal extends ScGunsAttackGoal<CrossBowmanEntity> {
    private int seeTime;

    public RecruitScGunsGunAttackGoal(CrossBowmanEntity shooter, double stopRange) {
    ...
    public void tick() {
        ...
        if (target != null && target.isAlive()) {
            ...
            if (this.shooter.getShouldHoldPos() && this.shooter.getHoldPos() != null && !this.shooter.getHoldPos().closerThan(this.shooter.getOnPos().getCenter(), 5.0)) {
                this.shooter.setAggressive(true);
                this.shooter.getNavigation().moveTo(target, this.speedModifier);
            }
        }
