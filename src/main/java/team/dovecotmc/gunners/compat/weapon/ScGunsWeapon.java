...
public class ScGunsWeapon implements IWeapon {
    ...
    public ScGunsWeapon(ItemStack stack) {
        if (stack.getItem() instanceof GunItem g) {
            ...
            this.gunStack = ModItems.REPEATING_MUSKET.get().getDefaultInstance();
            this.gun = ModItems.REPEATING_MUSKET.get().getGun();
        }
    }
    ...
    public int getWeaponLoadTime() {
        return GunEnchantmentHelper.getReloadInterval(gunStack);
    }
    ...
    private Item getAmmo() {
        if (ammo == null)
            ammo = gun.getProjectile().getItem();
        return ammo;
    }
}
